...
def claasp_sparkle_384(self: Cipher, input: ComponentState, number_of_steps):
    self.add_round()

    # state initialization
    state = []
    for i in range(2 * 6):
        state.append(ComponentState(input.id, [[k + i * WORD_SIZE for k in range(WORD_SIZE)]]))
    ...
    for r in range(number_of_steps):
        # round function
        state = round_function(self, state, constant_ci, constant_r[r], r)
    ...
    self.add_concatenate_component(ids, pos,256)
    state_left = ComponentState([self.get_current_component_id()], [list(range(256))])

    inputs = []
    for i in range(8, 12):
        inputs.append(deepcopy(state[i]))
    ids, pos = get_inputs_parameter(inputs)
    self.add_concatenate_component(ids, pos,128)
    state_right = ComponentState([self.get_current_component_id()], [list(range(128))])

    return state_left, state_right

def alzette(self, state_x, state_y, ci):
    for i in range(4):
        state_x, state_y = alzette_round(self, state_x, state_y, AZ_ROTATE[i][1], AZ_ROTATE[i][0], ci)
...
def round_function(self, state, constant_ci, constant_r, r):
    ...
    for i in range(len(state) // 2):
        state[2 * i], state[2 * i + 1] = alzette(self, state[2 * i], state[2 * i + 1], constant_ci[i])

    # Diffusion Layer
    # state = L(S)
    state = linear_layer(self, state)
